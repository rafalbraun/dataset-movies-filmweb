...
def find_movie(descr_id, synopsis):
    ...
    result = result_links[0] if len(result_links) == 1 else "z"
    return result
...
def update_database_with_links(conn, limit=1):
    ...
    """
    cursor = conn.cursor()
    cursor.execute(f"SELECT descr_id, synopsis FROM movies WHERE filmweb_url IS NULL OR filmweb_url = '' limit {limit}")
...
def main():
    ...
    """
    while True:
        if not is_within_time_range(0, 24):
